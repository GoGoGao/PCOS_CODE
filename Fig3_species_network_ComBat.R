...
combat_root <- normalizePath(file.path(root_dir, "..", "ComBat_network_check"))
...
SUBTITLE_NET <- sprintf(
  "CLR–ComBat; Spearman |r|>=%.1f, P<%.2f; top-250 taxa; Louvain modules; size∝abundance",
...
pair_map <- list(
  A = list(cohort = "overall",     title = "Overall (ComBat)"),
  B = list(cohort = "PRJNA549764", title = "PRJNA549764 (ComBat)"),
  C = list(cohort = "PRJNA530971", title = "PRJNA530971 (ComBat)"),
  D = list(cohort = "PRJNA791492", title = "PRJNA791492 (ComBat)")
)
pair_plots <- list()
...
fig_combined <- (leg_strip / net_grid / tab_G$plot) +
  ...
  plot_annotation(
    title = "Cohort co-occurrence networks after CLR–ComBat batch correction",
    theme = theme(
...
p_e <- plot_node_props(
  df_e,
  sprintf("Overall node properties (ComBat; |r|≥%.1f)", R_THRESHOLD)
)
...
p_f <- ggplot(df_f, aes(x = group, y = value)) +
  ...
  labs(
    title = sprintf(
      "Cohort-stratified node properties (ComBat; |r|≥%.1f)", R_THRESHOLD
    ),
...
p_deg <- ggplot(mean_deg, aes(x = cohort, y = avg_degree, fill = group)) +
  ...
  labs(
    title = sprintf(
      "Mean degree after CLR–ComBat (|r|≥%.1f)", R_THRESHOLD
    ),
...
fig3_full <- (
  ...
  plot_annotation(
    title = sprintf(
      "Figure 3. Microbial co-occurrence networks (CLR–ComBat; |r|≥%.1f)",
      R_THRESHOLD
    ),
