...
def parse_args():
    p = argparse.ArgumentParser(description='Module 5 (Species): Core Microbiome Analysis at Species Level', formatter_class=argparse.RawDescriptionHelpFormatter, epilog=__doc__)
    p.add_argument('--species', '-s', required=True, help='Species relative abundance table (TSV, species x samples)')
    ...
    p.add_argument('--prevalence', type=float, default=0.7, help='Primary prevalence threshold for core species (default: 0.70)')
    p.add_argument('--min_prev', type=float, default=0.1, help='Pre-filter: remove species present in < this fraction of samples (default: 0.10)')
    p.add_argument('--min_abund', type=float, default=0.01, help='Pre-filter: remove species with mean abundance < this value (%%) (default: 0.01, matches Module 1 preprocessing)')
...
def main():
    ...
    logging.basicConfig(level=logging.DEBUG if args.verbose else logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s', handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler(os.path.join(args.output, 'M5_species_core_log.txt'), mode='w')])
    pal = PALETTES[args.palette]
    colors_group = {'PCOS': pal[0], 'Healthy': pal[1]}
    colors_batch = {}
    logging.info('[1/7] Loading data')
    species_df = pd.read_csv(args.species, sep='\t', index_col=0)
    species_df = species_df.clip(lower=0)
    logging.info('  Raw species table: %d species x %d samples', *species_df.shape)
    ...
    common = sorted(set(species_df.columns) & set(group_info['Sample']))
    ...
    global_prev = (species_df > 0).sum(axis=1) / species_df.shape[1]
    global_abund = species_df.mean(axis=1)
    keep = (global_prev >= args.min_prev) & (global_abund >= args.min_abund)
    species_filt = species_df.loc[keep].copy()
    ...
    for thresh in thresholds:
        core_all, _, _ = identify_core(species_filt, common, thresh)
        core_pcos, _, _ = identify_core(species_filt, pcos_samples, thresh)
        core_healthy, _, _ = identify_core(species_filt, healthy_samples, thresh)
    ...
    logging.info('[4/7] Computing species statistics')
    _, prev_pcos, abund_pcos = identify_core(species_filt, pcos_samples, 0)
    _, prev_healthy, abund_healthy = identify_core(species_filt, healthy_samples, 0)
    species_stats = pd.DataFrame({'Species': species_filt.index, 'PCOS_prevalence': prev_pcos.values, 'Healthy_prevalence': prev_healthy.values, 'PCOS_abundance': abund_pcos.values, 'Healthy_abundance': abund_healthy.values})
    species_stats['Prevalence_diff'] = species_stats['PCOS_prevalence'] - species_stats['Healthy_prevalence']
    species_stats['Core_status'] = 'Non-core'
    species_stats.loc[species_stats['Species'].isin(shared_core), 'Core_status'] = 'Shared'
    species_stats.loc[species_stats['Species'].isin(pcos_specific), 'Core_status'] = 'PCOS-specific'
    species_stats.loc[species_stats['Species'].isin(healthy_specific), 'Core_status'] = 'Healthy-specific'
    ...
    for batch in batches:
        ...
        for grp_name, grp_samples in [('PCOS', b_pcos), ('Healthy', b_healthy)]:
            if len(grp_samples) >= 5:
                core_b, _, _ = identify_core(species_filt, grp_samples, args.prevalence)
                batch_core[f'{batch}_{grp_name}'] = set(core_b)
                logging.info('  %s_%s: %d core species', batch, grp_name, len(core_b))
    ...
    save_figure(fig_ab, combined_ab, os.path.join(args.output, 'M5S_Fig1_AB_venn_threshold'), dpi=args.dpi)
    plt.close(fig_ab)
    fig_a, ax = plt.subplots(figsize=(SINGLE_W_MM, SINGLE_W_MM))
    venn2([core_pcos_set, core_healthy_set], set_labels=('PCOS Core', 'Healthy Core'), set_colors=(colors_group['PCOS'], colors_group['Healthy']), alpha=0.65, ax=ax)
    ax.set_title(f'Core Species Overlap (Prevalence ≥ {prim}%)', fontsize=10, fontweight='bold')
    save_figure(fig_a, venn_plot_df, os.path.join(args.output, 'M5S_Fig1_panelA_venn'), dpi=args.dpi)
    plt.close(fig_a)
    ...
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    save_figure(fig_b, thresh_plot_df, os.path.join(args.output, 'M5S_Fig1_panelB_threshold'), dpi=args.dpi)
    plt.close(fig_b)
    ...
    for i, (grp, grp_core, grp_col) in enumerate([('PCOS', core_pcos_set, 'PCOS_prevalence'), ('Healthy', core_healthy_set, 'Healthy_prevalence')]):
        ...
        in_core = species_stats['Species'].isin(grp_core)
        c_arr = [colors_group[grp] if v else '#CCCCCC' for v in in_core]
        s_arr = [55 if v else 18 for v in in_core]
        ax.scatter(species_stats[grp_col] * 100, np.log10(species_stats[col_abund] + 0.001), c=c_arr, s=s_arr, alpha=0.65, edgecolors='white', linewidths=0.2)
        ...
        ax.set_title(f'(C{i + 1}) {grp} Group (core species: {len(grp_core)})', fontsize=10, fontweight='bold')
        ax.legend(fontsize=7)
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
    plt.tight_layout()
    prev_abund_df = species_stats[['Species', 'PCOS_prevalence', 'Healthy_prevalence', 'PCOS_abundance', 'Healthy_abundance', 'Core_status']].copy()
    save_figure(fig_c, prev_abund_df, os.path.join(args.output, 'M5S_Fig2_C_prevalence_abundance'), dpi=args.dpi)
    plt.close(fig_c)
    for i, (grp, grp_core, prev_col, abund_col) in enumerate([('PCOS', core_pcos_set, 'PCOS_prevalence', 'PCOS_abundance'), ('Healthy', core_healthy_set, 'Healthy_prevalence', 'Healthy_abundance')]):
        ...
        c_arr = [colors_group[grp] if v else '#CCCCCC' for v in in_core]
        s_arr = [55 if v else 18 for v in in_core]
        ax.scatter(species_stats[prev_col] * 100, np.log10(species_stats[abund_col] + 0.001), c=c_arr, s=s_arr, alpha=0.65, edgecolors='white', linewidths=0.2)
        ax.axvline(x=args.prevalence * 100, color='#333333', linestyle='--', linewidth=0.9, label=f'{prim}% threshold')
        ax.set_xlabel('Prevalence (%)', fontsize=9)
        ax.set_ylabel('log₁₀(Mean Abundance + 0.001)', fontsize=9)
        ax.set_title(f'{grp} Group (core species: {len(grp_core)})', fontsize=10, fontweight='bold')
        ax.legend(fontsize=7)
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        sub_df = species_stats[['Species', prev_col, abund_col, 'Core_status']].copy()
        save_figure(fig_ci, sub_df, os.path.join(args.output, f'M5S_Fig2_panelC{i + 1}_{grp.lower()}_scatter'), dpi=args.dpi)
        plt.close(fig_ci)
    shared_list = sorted(shared_core)
    shared_stats = species_stats[species_stats['Species'].isin(shared_list)].copy()
    shared_stats['Total_abund'] = shared_stats['PCOS_abundance'] + shared_stats['Healthy_abundance']
    top_n = min(30, len(shared_stats))
    top_shared = shared_stats.nlargest(top_n, 'Total_abund')['Species'].tolist()
    if top_shared:
        sample_order = pcos_samples + healthy_samples
        hm_data = species_filt.loc[top_shared, sample_order]
        ...
        hm_plot_df.index.name = 'Species'
        hm_plot_df = hm_plot_df.reset_index()
        hm_plot_df = pd.melt(hm_plot_df, id_vars='Species', var_name='Sample', value_name='Zscore')
        hm_plot_df = hm_plot_df.merge(group_info[['Sample', 'Group', 'Bioproject']], on='Sample', how='left')
        save_figure(fig_d, hm_plot_df, os.path.join(args.output, 'M5S_Fig3_D_shared_core_heatmap'), dpi=args.dpi)
        plt.close(fig_d)
    gs_sorted = species_stats.sort_values('Prevalence_diff', ascending=True)
    top_n_diff = 15
    ...
    ylabels = [g.replace('_', ' ')[:28] for g in top_diff['Species']]
    ax.set_yticks(y_pos)
    ax.set_yticklabels(ylabels, fontsize=7)
    ax.axvline(x=0, color='black', linewidth=0.8)
    ax.set_xlabel('Prevalence Difference (PCOS − Healthy, %)', fontsize=9)
    ax.set_title(f'(E) Top Species with Largest Prevalence Differences\n(PCOS vs Healthy)', fontsize=10, fontweight='bold')
    ...
    ax.spines['right'].set_visible(False)
    plt.tight_layout()
    save_figure(fig_e, top_diff[['Species', 'PCOS_prevalence', 'Healthy_prevalence', 'Prevalence_diff', 'Core_status']].copy(), os.path.join(args.output, 'M5S_Fig4_E_prevalence_diff'), dpi=args.dpi)
    plt.close(fig_e)
    if batch_core:
        ...
        if len(consistent) > 0:
            ...
            ax.set_title(f'(F) Cross-batch Core Species Consistency\n(Present in ≥{min_batches} batch-group combinations)', fontsize=10, fontweight='bold')
            plt.tight_layout()
            consist_plot = consistent.copy()
            consist_plot.index.name = 'Species'
            consist_plot = consist_plot.reset_index()
            save_figure(fig_f, consist_plot, os.path.join(args.output, 'M5S_Fig5_F_crossbatch_consistency'), dpi=args.dpi)
            plt.close(fig_f)
    ...
    ax0.set_title(f'(A) Core Species Overlap (≥{prim}%)', fontsize=9, fontweight='bold')
    ...
    ax1.set_ylabel('N Core Species', fontsize=8)
    ax1.set_title('(B) Core Size by Threshold', fontsize=9, fontweight='bold')
    ax1.legend(fontsize=7)
    ax1.spines['top'].set_visible(False)
    ax1.spines['right'].set_visible(False)
    ax2 = fig_comp.add_subplot(gs_outer[1, 0])
    in_core = species_stats['Species'].isin(core_pcos_set)
    c_arr = [colors_group['PCOS'] if v else '#CCCCCC' for v in in_core]
    s_arr = [45 if v else 12 for v in in_core]
    ax2.scatter(species_stats['PCOS_prevalence'] * 100, np.log10(species_stats['PCOS_abundance'] + 0.001), c=c_arr, s=s_arr, alpha=0.65, edgecolors='white', linewidths=0.15)
    ...
    in_core = species_stats['Species'].isin(core_healthy_set)
    c_arr = [colors_group['Healthy'] if v else '#CCCCCC' for v in in_core]
    s_arr = [45 if v else 12 for v in in_core]
    ax3.scatter(species_stats['Healthy_prevalence'] * 100, np.log10(species_stats['Healthy_abundance'] + 0.001), c=c_arr, s=s_arr, alpha=0.65, edgecolors='white', linewidths=0.15)
    ...
    ylabels2 = [g.replace('_', ' ')[:22] for g in top_diff['Species']]
    ...
    for line in summary_lines:
        ...
        y_start -= 0.053
    fig_comp.suptitle('Species-level Core Microbiome Analysis\nPCOS vs Healthy Gut Microbiome (3 cohorts, MetaPhlAn4)', fontsize=10, fontweight='bold', y=1.01)
    save_figure(fig_comp, combined_ab, os.path.join(args.output, 'M5S_Fig_composite'), dpi=args.dpi)
    plt.close(fig_comp)
    logging.info('[7/7] Saving result tables')
    species_stats.to_csv(os.path.join(args.output, 'M5S_species_prevalence_stats.tsv'), sep='\t', index=False)
    core_list_rows = []
    for grp_name, gset in [('PCOS-specific', pcos_specific), ('Healthy-specific', healthy_specific), ('Shared', shared_core)]:
        for g in sorted(gset):
            row = species_stats[species_stats['Species'] == g].iloc[0]
            core_list_rows.append({'Species': g, 'Core_status': grp_name, 'PCOS_prevalence': f"{row['PCOS_prevalence']:.4f}", 'Healthy_prevalence': f"{row['Healthy_prevalence']:.4f}", 'PCOS_abundance': f"{row['PCOS_abundance']:.4f}", 'Healthy_abundance': f"{row['Healthy_abundance']:.4f}", 'Prevalence_diff': f"{row['Prevalence_diff']:.4f}"})
    core_df = pd.DataFrame(core_list_rows)
    core_df.to_csv(os.path.join(args.output, 'M5S_core_species_list.tsv'), sep='\t', index=False)
    if batch_core:
        consist_out = consistent.copy() if len(consistent) > 0 else pd.DataFrame()
        if not consist_out.empty:
            consist_out.index.name = 'Species'
            consist_out.to_csv(os.path.join(args.output, 'M5S_crossbatch_consistent_species.tsv'), sep='\t')
    logging.info('=' * 60)
    logging.info('ANALYSIS SUMMARY')
    logging.info('=' * 60)
    logging.info('Species table (filtered): %d species x %d samples', species_filt.shape[0], species_filt.shape[1])
    logging.info('Primary threshold: %d%%', prim)
    logging.info('PCOS core species:    %d', len(core_pcos_set))
    logging.info('Healthy core species: %d', len(core_healthy_set))
    ...
    if top_shared:
        logging.info('Top 5 shared core species (by mean abundance):')
        for g in shared_stats.nlargest(5, 'Total_abund')['Species'].tolist():
            row = species_stats[species_stats['Species'] == g].iloc[0]
